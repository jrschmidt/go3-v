...
var Board, BoardBase, BoardLines, ClickHandler, GameCanvas, ServerConnection, Zipper, mousedown, start;

Zipper = (function() {

  function Zipper() {
    this.bb = new Board();
    this.clickster = new ClickHandler(this.bb, this.bb.drawing_object, this.connection);
  }
...
Board = (function() {
  ...
  function Board() {
    var row, rr, ss, _i, _j, _ref, _ref1;
    this.get_board_constants();
    this.drawing_object = new GameCanvas(this);
    this.points = [];
    for (rr = _i = 0; _i <= 10; rr = ++_i) {
      row = [];
      for (ss = _j = _ref = this.row_start[rr], _ref1 = this.row_end[rr]; _ref <= _ref1 ? _j <= _ref1 : _j >= _ref1; ss = _ref <= _ref1 ? ++_j : --_j) {
        row[ss] = 0;
      }
      this.points[rr] = row;
    }
...
ClickHandler = (function() {
  ...
  function ClickHandler(game, canvas_object, connection) {
    this.game = game;
  ...
  ClickHandler.prototype.click_handle = function(x, y) {
    ...
    if (this.game.legal_move(point)) {
      ...
      msg = String(point);
      this.connection = new ServerConnection(msg);
...
ServerConnection = (function() {
  ...
  ServerConnection.prototype.receive = function() {
    ...
    return alert("RESPONSE: " + this.xhr.responseText);
  };
